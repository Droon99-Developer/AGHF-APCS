...
public class DirectionPanel extends JPanel {
	...
	private void objectives() {
		...
		// "The goal of the game is to destroy the base on the opposite side of
		// the map as yours. /n This is accomplished by sending military units
		// to war with the other player. /n Diplomacy doesn/'t work, so don/'t
		// /try it.");
	...
	private void turn() {
		...
		instructions.add(new JLabel());
		instructions.get(0).setText(
				"1. Income: This is the phase where you get the gold from killing opposing units and from your");
		instructions.add(new JLabel());
		instructions.get(1).setText(" fixed income.");
		instructions.add(new JLabel());
		instructions.get(2).setText(
				"2. Buying: This is the phase where you buy your units for a specific price. The Units are on the next");
		instructions.add(new JLabel());
		instructions.get(3).setText(" page of the instructions");
		instructions.add(new JLabel());
		instructions.get(4).setText(
				"3. Waiting: The other player will now do steps 1 and 2. This may have happened before you did your ");
		instructions.add(new JLabel());
		instructions.get(5).setText("steps 1 and 2.");
		instructions.add(new JLabel());
		instructions.get(6).setText(
				"4. Skirmishes/Moving: All units will be deployed and attack opposing units or the enemy base,");
		instructions.add(new JLabel());
		instructions.get(7)
				.setText(" depending on where they are.  If the unit is unable to move before attacking, it will not");
		instructions.add(new JLabel());
		...
		for (int i = 0; i < instructions.size(); i++) {
			...
			instructions.get(i).setVisible(true);
			if (i == 8) {
				instructions.get(i).setLocation(50 + this.getWidth() / 2 - instructions.get(i).getWidth() / 2,
						slideTitle.getHeight() - 60 + 80 * (i / 2 + 1));
			} else if (i % 2 == 0) {
				instructions.get(i).setLocation(this.getWidth() / 2 - instructions.get(i).getWidth() / 2,
						slideTitle.getHeight() + 40 + 80 * (i / 2));
			} else if (i % 2 == 1) {
				instructions.get(i).setLocation(40 + this.getWidth() / 2 - instructions.get(i).getWidth() / 2,
						slideTitle.getHeight() - 10 + 80 * (i / 2 + 1));
			}
			add(instructions.get(i));
		}
	}
	...
	private void unitTable() {
		...
		Object[][] data = { { "Vehicle", "Air Strike", "Tank", "Infantry", "Marine", "Medic" },
				{ "Speed*", 5, 1, 2, 4, 1 },
				{ "Strength", 40, 7, 3, 1, "n/a**" }, { "Health", 40, 10, 7, 2, 7 }, { "Cost", Economy.AIRSTRIKECOST,
						Economy.TANKCOST, Economy.INFANTRYCOST, Economy.SCOUTCOST, Economy.MEDICCOST },
				{ "Reward", 10, 5, 3, 1, 4 } };

		units = new JTable(data, columnNames);
		units.setBounds((this.getWidth() / 2 - units.getWidth()) / 2 - 150,
				(this.getHeight() / 2 - units.getHeight()) / 2, 1000, 275);
	...
	private void shortcuts() {
		...
		Object[][] data = { { "Operation", "Key" }, { "Upgrade Base", "-" }, { "End Turn", "[Enter]" },
				{ "Show Units Panel", "=" }, { "Buy Air Strike", "1" }, { "Buy Tank", "2" }, { "Buy Marine", "3" },
				{ "Buy Infantry", "4" }, { "Buy Medic", "5" }, { "Scroll", "A and D or Left and Right Arrow Keys" } };

		KeyShorts = new JTable(data, columnNames);
		KeyShorts.setBounds(this.getWidth() / 2 - units.getWidth() / 2 + 0, slideTitle.getHeight() + 50,
				this.getWidth() - 495, this.getHeight() - 450);
